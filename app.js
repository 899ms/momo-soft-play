...
cardBlob=await new Promise(resolve=>c.toBlob(resolve,'image/jpeg',.94));if(!cardBlob)throw new Error('Card generation failed');if(cardURL)URL.revokeObjectURL(cardURL);cardURL=URL.createObjectURL(cardBlob);$('#card-image').src=cardURL;
}
...
$('#download').onclick=async()=>{if(!cardBlob)return;const name='MOMO-'+today()+'.jpg';try{const file=new File([cardBlob],name,{type:'image/jpeg'});if(navigator.canShare?.({files:[file]})){await navigator.share({title:'MOMO 收藏卡',files:[file]});$('#card-dialog').close();toast('收藏卡已交给手机，可以保存到照片');return;}const a=document.createElement('a');a.href=cardURL;a.download=name;document.body.appendChild(a);a.click();a.remove();$('#card-dialog').close();toast('JPG 收藏卡已开始保存');}catch(e){if(e.name!=='AbortError')$('#share-status').textContent='请长按上面的卡片，选择保存图片';}};
