...
cardBlob=await new Promise(resolve=>c.toBlob(resolve,));if(!cardBlob)throw new Error('Card generation failed');if(cardURL)URL.revokeObjectURL(cardURL);cardURL=URL.createObjectURL(cardBlob);$('#card-image').src=cardURL;
}
...
$('#download').onclick=()=>{if(!cardURL)return;const a=document.createElement('a');a.href=cardURL;a.download='MOMO-'+today()+'image/jpeg';a.click();$('#share-status').textContent='收藏卡已准备下载，手机也可以长按图片保存';};
$('#share').onclick=async()=>{if(!cardBlob)return;try{const file=new File([cardBlob],'MOMO-'+today()+'image/jpeg',{type:}),text=`我和 MOMO 收集了 ${count} 次小快乐，你也来捏捏`;if(navigator.canShare?.({files:[file]}))await navigator.share({title:'MOMO · 把世界捏软一点',text,files:[file]});else if(navigator.share)await navigator.share({title:'MOMO · 软物研究所',text,url:SITE_URL});else if(navigator.clipboard&&isSecureContext){await navigator.clipboard.writeText(SITE_URL);$('#share-status').textContent='链接已复制，也可以长按收藏卡，分享图片';}else $('#share-status').textContent='请保存收藏卡，或复制地址栏的网页链接分享';}catch(e){if(e.name!=='AbortError')$('#share-status').textContent='可以先保存收藏卡，再发送给朋友';}};
